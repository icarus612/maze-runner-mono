const Maze = (maze = {}) => {
	let m = {
		...
		buildNew(h = 10, w = 10, mazeType = "h") {
			...
			const makePos = (b) => Math.floor(Math.random() * (b - 2)) + 1
			if (mazeType == "h") {
				s = [1, makePos(this.width)];
				openPoints = openPoints.filter((i) => i[0] !== s[0] || i[1] !== s[1]);
				e = [this.height - 2, makePos];
				openPoints = openPoints.filter((i) => i[0] !== e[0] || i[1] !== e[1]);
			} else if (mazeType == "v") {
				s = [makePos(this.height), 1];
				openPoints = openPoints.filter((i) => i[0] !== s[0] || i[1] !== s[1]);
				e = [makePos(this.height), this.width - 2];
				openPoints = openPoints.filter((i) => i[0] !== e[0] || i[1] !== e[1]);
			} else if (mazeType == "r") {
				while (s[0] == e[0] && s[1] == e[1]) {
					s = [makePos(this.height), makePos(this.width)];
					e = [makePos(this.height), makePos(this.width)];
				}
				openPoints = openPoints.filter((i) => i[0] !== s[0] || i[1] !== s[1]);
				openPoints = openPoints.filter((i) => i[0] !== e[0] || i[1] !== e[1]);
